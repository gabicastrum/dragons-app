...
const mockBuscarDragaoPorId = buscarDragaoPorId as unknown as ReturnType<typeof vi.fn>;
...
const DRAGAO_MOCK = {
  ...
  histories: ["Tem escamas com aparência de cristais de gelo", "Nasceu nas montanhas de gelo"],
};
...
describe("DetailsPage", () => {
  ...
  it("deve exibir as histórias do dragão", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Tem escamas com aparência de cristais de gelo")).toBeInTheDocument();
      expect(screen.getByText("Nasceu nas montanhas de gelo")).toBeInTheDocument();
    });
  });
  ...
  it("deve exibir a data formatada em pt-BR", async () => {
    ...
    const dataFormatada = new Date(DRAGAO_MOCK.createdAt).toLocaleString("pt-BR");
